...
def display_graph(data,alpha,version):
    ...
    plt.title(f"{version} Fitness Over Generations")
    plt.xlabel('Timestep')
...
def graph_comparison():
    df_v1 = pd.read_csv('normal_brain_generation_fitness.csv', header=None)
    df_v2 = pd.read_csv('brainiac_brain_generation_fitness20.csv', header=None)
    ...
    plt.title('Fitness Over Generations Comparison: Brainiac vs. Normal Brain')
    plt.xlabel('Timestep')
    plt.ylabel('Generation')
    plt.grid(True, linestyle='--', alpha=0.5)
...
def graph_comparison_avg():
    ...
    plt.title('Fitness Over Generations Comparison: normal brain 1 vs. brainiac with averages')
    plt.xlabel('Timestep')
    plt.ylabel('Generation')
    plt.grid(True, linestyle='--', alpha=0.5)
